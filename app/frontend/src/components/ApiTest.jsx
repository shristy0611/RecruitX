...
const ApiTest = () => {
  ...
  const renderTestResult = (status, title) => {
    return (
      <div className="card mb-4">
        <h2 className="text-xl font-semibold mb-4 text-high-contrast">{title}</h2>
        <div className={`p-3 rounded-lg mb-4 ${
          status.loading ? 'bg-yellow-50 text-yellow-700 border border-yellow-200 dark:bg-yellow-900 dark:text-yellow-200 dark:border-yellow-800' :
          status.success ? 'bg-green-50 text-green-700 border border-green-200 dark:bg-green-900 dark:text-green-200 dark:border-green-800' :
        ...
        {status.details && (
          <div className="mt-4 p-4 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-auto">
            <pre className="whitespace-pre-wrap text-high-contrast">{JSON.stringify(status.details, null, 2)}</pre>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold mb-6 text-high-contrast">API Connection Test</h1>
      ...
      <div className="card mb-4">
        <h2 className="text-xl font-semibold mb-4 text-high-contrast">File Upload Test</h2>
        <p className="mb-4 text-medium-contrast">Upload a sample resume or job description file to test the file upload API:</p>
        
        <FileUpload onFileUpload={handleFileUpload} />
        
        {uploadStatus.loading && (
          <div className="p-3 bg-yellow-50 text-yellow-700 border border-yellow-200 rounded-lg dark:bg-yellow-900 dark:text-yellow-200 dark:border-yellow-800">
        ...
        {!uploadStatus.loading && uploadStatus.message !== 'Ready for upload test' && (
          <div className={`p-3 rounded-lg mt-4 ${
            uploadStatus.success ? 'bg-green-50 text-green-700 border border-green-200 dark:bg-green-900 dark:text-green-200 dark:border-green-800' : 
            'bg-red-50 text-red-700 border border-red-200 dark:bg-red-900 dark:text-red-200 dark:border-red-800'
          }`}>
            <p>
        ...
        {uploadStatus.details && (
          <div className="mt-4 p-4 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-auto">
            <pre className="whitespace-pre-wrap text-high-contrast">{JSON.stringify(uploadStatus.details, null, 2)}</pre>
          </div>
        )}
      </div>
      
      <div className="card">
        <h3 className="font-semibold mb-2 text-high-contrast">Debug Information:</h3>
        <ul className="list-disc list-inside space-y-1 text-medium-contrast">
          <li>Backend URL: <code className="bg-gray-100 dark:bg-gray-700 px-1 rounded text-high-contrast">http://localhost:8000</code></li>
          <li>Proxy Settings: <code className="bg-gray-100 dark:bg-gray-700 px-1 rounded text-high-contrast">/api → http://localhost:8000</code></li>
          <li>Testing Mode: <code className="bg-gray-100 dark:bg-gray-700 px-1 rounded text-high-contrast">{testStatus.details?.testMode ? 'Enabled' : 'Disabled'}</code></li>
          <li>Browser: <code className="bg-gray-100 dark:bg-gray-700 px-1 rounded text-high-contrast">{navigator.userAgent}</code></li>
        </ul>
      </div>
    </div>
  );
